...
# так вот что делает скрипт
# 1. Проверяет входные данные 
# 2. Создает папку для деплоя
# 3. Копирует файлы приложения (которого пока нет)
# 4. Иммитирует перезапуск сервиса
# 5. Проверяет что все на месте
